...
class Program(customtkinter.CTk):
	...
	def __program_version__(self, event: str):
		if language_data == "Српски":
			showinfo(title="Version_info", message="версија: 6.0")

		elif language_data == "English":
			showinfo(title="Version_info", message="version: 6.0")

		else:
			showinfo(title="Version_info", message="версия: 6.0")
	...
	def __open_terminal__(self, event: str) -> None:
		try:
			from My_Diary_command_prompt_6 import Terminal
